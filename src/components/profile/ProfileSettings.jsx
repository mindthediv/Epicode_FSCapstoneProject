...
const ProfileSettings = () => {
    ...
    try {
      const response = await fetch(API_USERS, {
        ...
        body: JSON.stringify({
          profileImg: imgPic,
          userId: settingState.userId,
          username: reduxRegister.username,
          email: reduxRegister.email,
        }),
      });
      if (response.ok) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (reduxRegister.username == "") {
      dispatch(handleUsername(logged.username));
    }
    if (reduxRegister.email == "") {
      dispatch(handleUsername(logged.email));
    }
  ...
  return (
    <Container fluid id="profileSettings">
